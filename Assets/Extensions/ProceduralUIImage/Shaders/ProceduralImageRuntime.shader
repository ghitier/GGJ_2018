...
Shader "UI/Procedural UI Image"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			#ifdef BLUR_ON
				half2 viewPos	: TEXCOORD3;
			#endif
			};
			
			float4		_BlurBehindRect;
			sampler2D	_BlurBehindTex;
			half4		_BlurBackgroundColor;
			...
			v2f vert(appdata_t IN){
				...
				OUT.uv1 = IN.uv1;
				OUT.uv2 = IN.uv2 * float2(_Width,_Height);
...
//			}
			...
			fixed4 frag (v2f IN) : SV_Target
			{
				...
				color.rgb *= tex2D(_MainTex, IN.uv1).rgb;

			#ifdef BLUR_ON
				color.rgb = lerp(color.rgb * tex2D(_BlurBehindTex, IN.viewPos).rgb, _BlurBackgroundColor.rgb, _BlurBackgroundColor.a);
			#endif
				...
				if(_LineWeight>0){
					...
					color.a *= saturate((l-distance(visible(IN.uv2,_Radius),l))*_PixelWorldScale);
				}
				else{
					color.a *= saturate(visible(IN.uv2,_Radius)*_PixelWorldScale);
				}

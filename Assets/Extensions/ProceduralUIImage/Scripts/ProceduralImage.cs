...
namespace UnityEngine.UI.ProceduralImage {
	...
	public class ProceduralImage : Image {
		[SerializeField]private bool blur = false;
		...
		// This is based on Unity version : https://bitbucket.org/Unity-Technologies/ui/src/0bd08e22bc17bdf80bf7b997a4b43877ae4ee9ac/UnityEngine.UI/UI/Core/Image.cs?at=5.2&fileviewer=file-view-default
		void GenerateSimpleSprite(VertexHelper vh)
		{
			var r = GetPixelAdjustedRect();
			var v = new Vector4(r.x, r.y, r.x + r.width, r.y + r.height);
			...
			var uv2 = new Vector4 (0, 0, 1, 1);

			var color32 = color;
			vh.Clear();
			vh.AddVert(new Vector3(v.x, v.y), color32, new Vector2(uv.x, uv.y), new Vector2(uv2.x, uv2.y), Vector4.zero, Vector3.zero);
			vh.AddVert(new Vector3(v.x, v.w), color32, new Vector2(uv.x, uv.w), new Vector2(uv2.x, uv2.w), Vector4.zero, Vector3.zero);
			vh.AddVert(new Vector3(v.z, v.w), color32, new Vector2(uv.z, uv.w), new Vector2(uv2.z, uv2.w), Vector4.zero, Vector3.zero);
			vh.AddVert(new Vector3(v.z, v.y), color32, new Vector2(uv.z, uv.y), new Vector2(uv2.z, uv2.y), Vector4.zero, Vector3.zero);
		...
		public override Material GetModifiedMaterial (Material baseMaterial){
			...
			m = MaterialHelper.SetMaterialValues (new ProceduralImageMaterialInfo (rect.width + falloffDistance, rect.height + falloffDistance, Mathf.Max (pixelSize, 0), radius, Mathf.Max (borderWidth, 0), blur, backgroundColor), m);
			return m;
		}
	}
}
